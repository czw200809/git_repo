...
{	
	...
	//某个文件指定版本的内容
    public static byte[] getContent(Map<String, Object> rstMap, String gitRoot, String branchName, String revision, String filePath, Map<String, Object> fileMap) throws Exception
    {
    	boolean isNew = false;
    	if (rstMap == null)
    	{    		
    ...
    public static List<GitDirEntryDto> listDirEntry(String gitRoot, String branchName, String revision, String filePath) throws Exception
    {
        ...
        if (repo != null)
		{
        	...
        	if (revision == null)
        		...
        		commitId = repo.resolve(revision);
    	...
    	if (dirList != null)
    	{
    		...
    		for (Iterator<String> iter = dirList.iterator(); iter.hasNext();)
    		{
            	...
            	for (Iterator<GitLogDto> it = logDtoList.iterator(); it.hasNext();)
            	{
            		...
            		if (dto.commitDate.getTime() > rev1.getCommitterIdent().getWhen().getTime())
            		{
            			it.remove();
            		}
            	}
